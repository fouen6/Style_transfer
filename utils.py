...
def make_transform(size: tuple, normalize=True):
    ...
    :param size: 模型输入的图像尺寸 (宽, 高)
    :param normalize: 是否进行规范化（VGG的输入需要规范化）
    :return:
    """
    transform_lst = [transforms.Resize(size),  # 将图像大小调整为给定尺寸
                     transforms.ToTensor()]  # 将 PIL 图像转换为 Tensor

    if normalize:
        transform_lst.append(transforms.Normalize(mean=[0.485, 0.456, 0.406],
...
def save_image(tensor, output_dir, filename, denormalization=True):
    """
    保存图像到指定目录
    :param tensor: [0-1]区间的图像张量，形状为(1, 3, h, w)、(3, h, w)
    :param output_dir: 输出路径
    :param filename: 文件名
    :param denormalization: 是否进行反规范化
    :return:
    """
    ...
    # 处理 (1, 3, h, w) 或 (3, h, w) 两种输入
    if tensor.dim() == 4:
        image_tensor = tensor[0].cpu().detach()
    elif tensor.dim() == 3:
        image_tensor = tensor.cpu().detach()
    else:
        raise ValueError("Tensor shape 必须为 (1,3,H,W) 或 (3,H,W)")
